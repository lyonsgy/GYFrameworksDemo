...
- (NSString *)encryptString:(NSString *)stringToEncrypt{
    ...
    NSData *strData=[stringToEncrypt dataUsingEncoding:NSUTF8StringEncoding];
    Byte*bytes=(Byte *)[strData bytes];
	long iStringLength = [strData length]; // declare a variable to take the length of the incoming string
	unsigned char k;
	unsigned char t;
	
	// loop to grab all characters one at a time and send them through the array
	for (int c = 0; c < iStringLength; c++){
		k = ([self KSA]); // call the key stream selector// a temp variable to store the last taken in character from the array
        t=bytes[c];
